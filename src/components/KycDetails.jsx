...
const CustomerDetail = () => {
  ...
  const [updateKycStatus, { isLoading: isUpdating }] = useUpdateKycStatusMutation(); // Mutation for updating KYC status
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isImageModalOpen, setIsImageModalOpen] = useState(false);
  const [selectedImage, setSelectedImage] = useState(null);
  ...
  return (
    ...
    <div className="flex-grow">
      ...
      <p className="text-sm text-gray-600">
        Review moderation results including face match, OCR, and liveliness details.
      </p>
    </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div
          ...
          className={`p-6 rounded-lg shadow-lg cursor-pointer transform transition-all duration-300 ease-in-out hover:scale-105 ${
            ...
            selectedCard === "ocr" ? "ring-4 ring-green-300 scale-105" : "ring-2 ring-transparent"
          }`}
        >
        ...
        <div
          ...
          className={`p-6 rounded-lg shadow-lg cursor-pointer transform transition-all duration-300 ease-in-out hover:scale-105 ${
            ...
            selectedCard === "face" ? "ring-4 ring-blue-500 scale-105" : "ring-2 ring-transparent"
          }`}
        >
          <h3 className="text-lg font-bold mb-4 flex items-center">
          ...
          <p>{kyc.moderation.faceMatch.match ? "Matched" : "Not Matched"}</p>
        </div>
        ...
        <div
          ...
          className={`p-6 rounded-lg shadow-lg cursor-pointer transform transition-all duration-300 ease-in-out hover:scale-105 ${
            ...
            selectedCard === "liveliness" ? "ring-4 ring-yellow-300 scale-105" : "ring-2 ring-transparent"
          }`}
        >
          <h3 className="text-lg font-bold mb-4 flex items-center">
      ...
      {selectedCard === "liveliness" && !kyc?.moderation?.liveliness?.passed && (
        <div className="mb-8">
          ...
          <div className="mt-4 space-y-4">
            {Object.entries(kyc.moderation.liveliness.livelinessResults).map(([key, value]) => (
              <div key={key} className="flex justify-between">
                ...
                >
                  {value.passed ? "Passed" : "Failed"} (Score: {value.score})
                </span>
              </div>
            ))}
          </div>
...
const ActionButton = ({ buttonText, onClick, isLoading }) => (
  <button
    ...
    className={`px-5 py-2 rounded-lg font-semibold shadow-md ${
      isLoading ? "bg-gray-300" : buttonText === "Approve" ? "bg-green-500" : "bg-red-500"
    } text-white`}
    disabled={isLoading}
  >
